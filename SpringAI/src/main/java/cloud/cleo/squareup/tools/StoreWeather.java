...
public class StoreWeather extends AbstractTool {
    ...
    private StoreWeatherResponse unavailableResponseWithSummaryOnly(
        ...
        // We *did* get current conditions but forecast is busted.
        return new StoreWeatherResponse(
                "Wahkon, MN",
    ...
    public record StoreWeatherResponse(
            String locationName,
            String summary, // e.g. "Cloudy, breezy"
            double temperatureFahrenheit, // current temp (F)
            double windSpeedMilesPerHour, // current wind speed (mph)
            List<DailyForecast> forecast
            ) {

        public record DailyForecast(
                String date, // ISO date, local (America/Chicago)
                double minTempFahrenheit,
                double maxTempFahrenheit,
                String summary // e.g. "Light snow"
                ) {
